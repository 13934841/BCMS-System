...
# def authenticate_user(username: str, password: str, role):
#     try:
...
# # Define your dashboard endpoint
...
#         return HTMLResponse(content=f"<h1>Database Error: {e}</h1>")
...
#     rendered_template = template_content.format(username=current_user.username, data=data)
...
#         return RedirectResponse(url="/dashboard")  # Update the URL to your dashboard endpoint
#     else:
...
async def login_submit(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    if user_data:
        ...
        if permission == "Read":
            ...
            return RedirectResponse(url="/read_write_dashboard")
...
# async def full_access_dashboard():
#     # Read the content of the HTML template file
#     with open("full_access_dashboard.html", "r") as file:
#         template_content = file.read()
...
# async def full_access_dashboard_post(request: Request, current_user: User = Depends(get_current_user)):
#     # Handle POST requests to the full access dashboard (if needed)
